...
async def _generate_captions(
    ...
    prompt = (
        f"Platform(s): {platform_hint}{profile_block}{topic_block}{feedback_block}\n\n"
...
async def generate_image_endpoint(
    ...
    # Enqueue Celery task (lazy import so missing module doesn't break startup)
    try:
        ...
        logger.exception("ai_posts: could not enqueue generate_image_task: %s", exc)
...
async def regenerate_image(
    ...
    try:
        ...
        logger.exception("ai_posts: could not enqueue generate_image_task (regen): %s", exc)
