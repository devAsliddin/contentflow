...
async def instagram_oauth_callback(
    ...
    try:
        ...
        # Detached — .delay() is blocking broker I/O; a down/slow broker must not
        # stall the OAuth callback. beat/initial analysis is best-effort anyway.
        def _enqueue(account_id: str, job_id: str) -> None:
            from app.tasks.analysis_tasks import run_initial_analysis  # type: ignore[import]  # noqa: PLC0415
            run_initial_analysis.delay(account_id, job_id)

        fire_and_forget(
            lambda: _enqueue(str(account.id), str(analysis_job.id)),
