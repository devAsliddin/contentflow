...
async def start_analysis(
    ...
    # Enqueue Celery task (lazy import — analysis_tasks.py may not exist yet)
    try:
        ...
        logger.info("analysis.start enqueued job_id=%s account_id=%s", job.id, account_id)
    except Exception as exc:  # noqa: BLE001
        logger.warning("analysis.start could not enqueue task: %s", exc)
