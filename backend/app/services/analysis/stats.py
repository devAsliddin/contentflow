...
async def compute_stats(db: AsyncSession, account_id: uuid.UUID) -> StatsSummary:
    ...
    for item in media_items:
        ...
        # Hour bucket (weekday × hour). Convert to Tashkent local time so the
        # "best posting hours" the user sees are local clock times, not UTC.
        posted = to_local(item.posted_at)
